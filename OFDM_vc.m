...
%--------------------------发射端-------------------------------%
        ...
        % 插入虚拟子载波
        frame_withVC = [frame_mod(1:Nused/2, :); zeros(Nvc, Nframe); frame_mod(Nused/2+1:Nused, :)];
        % IFFT
        power_FT = sum(sum(abs(frame_mod).^2))/Nused/Nframe;  % 计算下IFFT前的能量，FT表示频域

        frame_mod_shift = ifftshift(frame_withVC);         % 频域归零
...
%--------------------------Channel-------------------------------%
        ...
        % 添加高斯白噪声
        power_TDserial = sum(abs(frame_TDserial).^2)/Nused/Nframe;     % 计算出的能量和理论不符啊，没发现问题在哪
        EsN0 = EbN0 + 10*log10(Npsk)+ 10*log10((Nused/Nfft));                                  % 根据信噪比计算噪声能量，幅值，然后加在信号上
...
%--------------------------接收端-------------------------------%
        ...
        % 去Nvc
        frame_noVC = [frame_equalization(1:Nused/2, :); frame_equalization(Nused/2+Nvc+1:Nk, :)];
        % QPSK解调
 
        frame_demod = QPSKDemod(frame_noVC, Nused, Nframe);
        % 并串转换
