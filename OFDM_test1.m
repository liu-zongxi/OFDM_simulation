...
for kk = 1:length(EbN0s)
    ...
    for ii = 1:nloop
        ...
        frame_ifft = ifftshift(ifft(frame_mod_shift, Nfft));             % ifft
        ...
        % Channel
        power_TDserial = sum(abs(frame_TDserial).^2)/N/Nframe;     % 计算出的能量和理论不符啊，没发现问题在哪
...
%         frame_recieved = frame_TDserial + noise_msg;
        ...
        % FFT
        frame_noGI_shift = fftshift(frame_noGI);
